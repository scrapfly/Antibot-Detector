...
(function() {
  ...
  class WindowPropertyTracker {
    ...
    startPolling() {
      ...
      this.currentPhase = POLLING_PHASES.EARLY;
    ...
    _scheduleNextPoll() {
      ...
      const totalDuration = POLLING_PHASES.EARLY.duration +
                           POLLING_PHASES.NORMAL.duration +
                           POLLING_PHASES.LATE.duration +
                           POLLING_PHASES.FINAL.duration;
    ...
    _determinePhase(elapsed) {
      if (elapsed < POLLING_PHASES.EARLY.duration) {
        return POLLING_PHASES.EARLY;
      } else if (elapsed < POLLING_PHASES.EARLY.duration + POLLING_PHASES.NORMAL.duration) {
        return POLLING_PHASES.NORMAL;
      } else if (elapsed < POLLING_PHASES.EARLY.duration + POLLING_PHASES.NORMAL.duration + POLLING_PHASES.LATE.duration) {
        return POLLING_PHASES.LATE;
      } else {
        return POLLING_PHASES.FINAL;
      }
    }
    ...
    _checkCondition(value, definition) {
      ...
      // Pre-defined condition evaluators (same as content-main-world.js)
      const evaluators = {
      ...
      // Unknown condition - default to truthy
      return !!value;
    }
    ...
    _scheduleRetry(path, trackingData, retryType) {
      const config = RETRY_CONFIG[retryType];
      if (!config) return;
